...
export const LessonFormTab = () => {
  ...
  const [lessonDetailFormDataList, setLessonDetailFormDataList] = useState<
    ...
    lessonDetailFormDataList.forEach((lesson, index) => {
      if (lesson.description.trim() == "") {
        newLessonErrorMessage[index].description =
          "Không được để trống mô tả nội dung khoá học.";
        errCount++;
      } else {
        newLessonErrorMessage[index].description = "";
      }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (errCount > 0) {
        toastLoadingFailAction(
          loadingId,
          "Vui lòng không để trống các trường cần thiết."
        );
        return;
      }
      ...
      if (response.data)
        toastLoadingSuccessAction(
          loadingId,
          "Tạo nội dung khoá học thành công!"
        );

      await fetchPortalDetail();
      handleCloseCourseModal();
    } catch (error) {
      toastLoadingFailAction(loadingId, "Không thể tạo nội dung khoá học");
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-danger">
                      {lessonErrorMessage[name]?.description}
                    </span>
                  </Form.Item>
                  ...
                  >
                    <Switch
                      ...
                      value={
                        lessonDetailFormDataList[name]?.trialLesson || false
                      }
                    />
                  </Form.Item>
