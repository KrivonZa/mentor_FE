...
import {
  ClassCreateRequest,
  ClassOverallQueryParam,
  ClassOverallResponse,
  ClassPortalOverallResposne,
  ClassUpdateRequest,
  CourseDetailResponse,
  CreateSessionRequest,
} from "../types/classModel";
...
const classService = {
  getClassPagination: async (
    req: ClassOverallQueryParam
  ): Promise<ApiResponse<Pagable<ClassOverallResponse>>> => {
    try {
  ...
  getClassPortalPagination: async (
    req: ClassOverallQueryParam
  ): Promise<ApiResponse<Pagable<ClassPortalOverallResposne>>> => {
    try {
      const response = await classPrivateApi.get(`/get-all-class-portal`, {
        params: req,
      });
      return response.data;
    } catch (error) {
  ...
  getClassStudent: async (id: number) => {
    try {
      const response = await classPrivateApi.get(`/get-all-students/${id}`);
      return response.data;
    } catch (error) {
      throw error;
    }
  ...
  setClassVisibility: async (
    id: number,
    status: boolean
  ): Promise<ApiResponse<any>> => {
    try {
      const response = await classPrivateApi.patch(
        `/set-class-visibility/${id}?status=${status}`
      );
      return response.data;
    } catch (error) {
      throw error;
